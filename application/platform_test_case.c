...
* @Last Modified time: 2016-12-19 21:09:29
*/
...
void pwm_test(void)
{
    ...
    pwm_init(PWM_CHANNEL_1 + PWM_CHANNEL_4, 10000);
...
void adc_sampling(void* param)
{
    ...
    if (duty_cycle >= 1000)
        duty_cycle = 0;
    uart_printf("Channel1: %d \t channel2: : %d\t adc: %d\n", \
                duty_cycle, 1000 - duty_cycle, adc_read_single_channel());
}
...
void i2c_test(void)
{
    gpio_init_function(GPIO_PIN(GPIO_PB, 8), AF1, OPEN_DRAIN, NO_PULL);
    gpio_init_function(GPIO_PIN(GPIO_PB, 9), AF1, OPEN_DRAIN, NO_PULL);
